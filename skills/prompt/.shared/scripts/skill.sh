...
cmd_validate() {
    ...
    if [[ ! -f "$INCLUDE_DIR/prompt_cli.py" ]]; then
        echo "error: missing $INCLUDE_DIR/prompt_cli.py" >&2
        errors=$((errors + 1))
    fi
    ...
    echo "ok: prompt skill is runnable"
}

# Dispatch to Python CLI
cmd_dispatch() {
    cd "$INCLUDE_DIR"
    uv run python prompt_cli.py "$@"
}
...
case "${1:-help}" in
    ...
    validate)
        cmd_validate
        ;;
    status|init|show|ready|compile|exec|receipts|clean)
        cmd_dispatch "$@"
        ;;
    *)
