...
function Show-Help {
    ...
    try {
        & uv run python sniff_scan_cli.py help
    } finally {
        Pop-Location
    }
}
...
function Invoke-Validate {
    ...
    Write-Output "ok: sniff-preventers skill is runnable"
}
...
function Invoke-Dispatch {
    ...
    Push-Location $IncludeDir
    try {
        & uv run python sniff_scan_cli.py @Arguments
    } finally {
        Pop-Location
    }
}
...
switch ($command) {
    ...
    { $_ -in @("scan", "clean") } { Invoke-Dispatch -Arguments $args }
    default {
        Write-Error "error: unknown command '$command'"
        Write-Output "run 'sniff-preventers help' for usage"
        exit 1
    }
}
