...
function Show-Help {
    ...
    try {
        & uv run python sniff_locate_cli.py help
    } finally {
        Pop-Location
    }
}
...
function Invoke-Validate {
    ...
    Write-Output "ok: sniff-locate skill is runnable"
}
...
function Invoke-Dispatch {
    ...
    Push-Location $IncludeDir
    try {
        & uv run python sniff_locate_cli.py @Arguments
    } finally {
        Pop-Location
    }
}
...
switch ($command) {
    ...
    { $_ -in @("list", "validate-findings", "reanchor", "clean") } { Invoke-Dispatch -Arguments $args }
    default {
        Write-Error "error: unknown command '$command'"
        Write-Output "run 'sniff-locate help' for usage"
        exit 1
    }
}
