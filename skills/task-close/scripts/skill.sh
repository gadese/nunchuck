...
cmd_validate() {
    ...
    if [[ ! -f "$INCLUDE_DIR/task_close_cli.py" ]]; then
        echo "error: missing $INCLUDE_DIR/task_close_cli.py" >&2
        errors=$((errors + 1))
    fi
    ...
    echo "ok: task-close skill is runnable"
}

cmd_run() {
    PYTHONPATH="$INCLUDE_DIR" uv run python "$INCLUDE_DIR/task_close_cli.py" "$@"
}
