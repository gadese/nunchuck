...
cmd_validate() {
    ...
    if [[ ! -f "$INCLUDE_DIR/grape_cli.py" ]]; then
        echo "error: missing $INCLUDE_DIR/grape_cli.py" >&2
        errors=$((errors + 1))
    fi
    ...
    echo "ok: grape skill is runnable"
}

cmd_dispatch() {
    cd "$INCLUDE_DIR"
    uv run python grape_cli.py "$@"
}
...
case "${1:-help}" in
    ...
    *)
        ...
        echo "run 'grape help' for usage" >&2
        exit 1
        ;;
esac
