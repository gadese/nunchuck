...
function Show-Help {
    ...
    try {
        & uv run python dtx_expand_cli.py help
    } finally {
        Pop-Location
    }
}
...
function Invoke-Validate {
    ...
    Write-Output "ok: dtx-expand skill is runnable"
}
...
function Invoke-Dispatch {
    ...
    Push-Location $IncludeDir
    try {
        & uv run python dtx_expand_cli.py @Arguments
    } finally {
        Pop-Location
    }
}
...
switch ($command) {
    ...
    { $_ -in @("expand", "clean") } { Invoke-Dispatch -Arguments $args }
    default {
        Write-Error "error: unknown command '$command'"
        Write-Output "run 'dtx-expand help' for usage"
        exit 1
    }
}
