...
function Show-Help {
    ...
    try {
        & uv run python dtx_forget_cli.py help
    } finally {
        Pop-Location
    }
}
...
function Invoke-Validate {
    ...
    Write-Output "ok: dtx-forget skill is runnable"
}
...
function Invoke-Dispatch {
    ...
    Push-Location $IncludeDir
    try {
        & uv run python dtx_forget_cli.py @Arguments
    } finally {
        Pop-Location
    }
}
...
switch ($command) {
    ...
    { $_ -in @("forget", "clean") } { Invoke-Dispatch -Arguments $args }
    default {
        Write-Error "error: unknown command '$command'"
        Write-Output "run 'dtx-forget help' for usage"
        exit 1
    }
}
