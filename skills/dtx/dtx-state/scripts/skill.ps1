...
function Show-Help {
    ...
    try {
        & uv run python dtx_state_cli.py help
    } finally {
        Pop-Location
    }
}
...
function Invoke-Validate {
    ...
    Write-Output "ok: dtx-state skill is runnable"
}
...
function Invoke-Dispatch {
    ...
    Push-Location $IncludeDir
    try {
        & uv run python dtx_state_cli.py @Arguments
    } finally {
        Pop-Location
    }
}
...
switch ($command) {
    ...
    "show" { Invoke-Dispatch -Arguments $args }
    default {
        Write-Error "error: unknown command '$command'"
        Write-Output "run 'dtx-state help' for usage"
        exit 1
    }
}
