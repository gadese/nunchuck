...
function Invoke-Validate {
    ...
    Write-Output "ok: changelog skill is runnable"
}
...
function Invoke-Dispatch {
    ...
    Push-Location $IncludeDir
    try {
        & uv run python changelog_cli.py @Arguments
    } finally {
        Pop-Location
    }
}
...
switch ($command) {
    ...
    { $_ -in @("locate", "init", "verify", "add", "release", "suggest", "clean") } {
        Invoke-Dispatch -Arguments $args
    }
    default {
        Write-Error "error: unknown command '$command'"
        Write-Output "run 'changelog help' for usage"
        exit 1
    }
}
